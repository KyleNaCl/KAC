...
local function is_bad_table(tbl)
	if not tbl then return false end
	if not tbl.size then return false end
	if tbl.size > 100 then
		return true
	elseif tbl.size > 0 then
...
local function force_end(reason, self2, force)
	KAC.printClient(self2.player:UserID(), -2, "Alert# '" .. (string.PatternSafe(self2.entity.name) or "generic") .. "' illegal " .. reason)
	hook.Run("KAC_Log_Wiremod","{1} E2 {2} illegal" .. reason,{GAS.Logging:FormatPlayer(self2.player),GAS.Logging:Highlight(string.PatternSafe(self2.entity.name) or "generic")})
	if force != nil then
		self2.entity:Error("Illegal " .. reason .. " [Stopped By: KAC]", "Stopped By: KAC")
		self2.entity:PCallHook('destruct')
	end
end
...
e2function table table(...)
	...
	for k, v in ipairs( tbl ) do
		...
		if blocked_types[tid] then
			...
			-- Recursive Table Check -s-
			if typeids[k] == "t" then
				if is_bad_table(v) then
					force_end("table()", self)
					return ret
				end
			end
			-- Recursive Table Check -e-
			size = size + 1
			ret.n[k] = v
			ret.ntypes[k] = tid
		end
...
e2function table table:difference( table rv2 )
	...
	for k,v in pairs( this.n ) do
		...
		if not rv2.n[k] then
			...
			-- Recursive Table Check -s-
			if is_bad_table(v) then
				force_end("table:difference()", self)
				return this
			end
			-- Recursive Table Check -e-
...
e2function table table:intersect( table rv2 )
	...
	for k,v in pairs( this.n ) do
		...
		if rv2.n[k] then
			...
			-- Recursive Table Check -s-
			if is_bad_table(v) then
				force_end("table:intersect()", self)
				return this
			end
			-- Recursive Table Check -e-
...
registerCallback( "postinit", function()
	...
	for k,v in pairs( wire_expression_types ) do
		...
		registerOperator("idx", id.."=ts"..id , id, function( self, args )
			...
			if id == "t" then
				if is_bad_table(rv3) then
					force_end("table[set,table]", self, true)
				end
			end
