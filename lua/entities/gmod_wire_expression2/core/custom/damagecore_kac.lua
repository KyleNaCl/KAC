...
E2Lib.RegisterExtension("damagecore_kac", true)
...
e2function void entity:dmgApplyDamage(number damage)
	...
	if KAC.InBuild(self.player) then
		return nil
	end

	damage = math.Clamp(damage, 0.001, 100000)
	...
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} executed {2} from {3} on {4}",{GAS.Logging:FormatPlayer(self.player),GAS.Logging:Highlight(tostring(damage)),GAS.Logging:Highlight(tostring(self.entity)),GAS.Logging:FormatEntity(this)})
	end
end
...
e2function void entity:takeDamage(damage dmg)
	...
	dmg.Damage = math.Clamp(dmg.Damage, 0.001, 100000)

	local dmginfo = tabtodamage(dmg)
	this:TakeDamageInfo(dmginfo)
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} executed {2} applied {3} using {4} on {5}",{GAS.Logging:FormatPlayer(self.player),GAS.Logging:Highlight(tostring(dmg.Attacker)),GAS.Logging:Highlight(tostring(dmg.Damage)),GAS.Logging:Highlight(tostring(dmg.Inflictor)),GAS.Logging:FormatEntity(this)})
	end
end
...
e2function void entity:takeDamage(number dmg)
	...
	dmg = math.Clamp(dmg, 0.001, 100000)
	
	this:TakeDamage(dmg, attacker, inflictor)
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} applied {2} using {3} on {4}",{GAS.Logging:FormatPlayer(attacker),GAS.Logging:Highlight(tostring(dmg)),GAS.Logging:Highlight(tostring(inflictor)),GAS.Logging:FormatEntity(this)})
	end
end
...
e2function void entity:takeDamage(number dmg, entity attacker)
	...
	this:TakeDamage(dmg, attacker, inflictor)
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} executed {2} applied {3} on {4}",{GAS.Logging:FormatPlayer(self.player),GAS.Logging:Highlight(tostring(attacker)),GAS.Logging:Highlight(tostring(dmg)),GAS.Logging:FormatEntity(this)})
	end
end
...
e2function void entity:takeDamage(number dmg, entity attacker, entity inflictor)
	...
	this:TakeDamage(dmg, attacker, inflictor)
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} executed {2} applied {3} using {4} on {5}",{GAS.Logging:FormatPlayer(self.player),GAS.Logging:Highlight(tostring(attacker)),GAS.Logging:Highlight(tostring(dmg)),GAS.Logging:Highlight(tostring(inflictor)),GAS.Logging:FormatEntity(this)})
	end
end
...
e2function void blastDamage(damage dmg, vector pos, number radius)
	...
	dmg.Damage = math.Clamp(dmg.Damage, 0.001, 100000)
	radius = math.Clamp(radius, 0.001, 5000)
	
	local dmginfo = tabtodamage(dmg)
	local pos = Vector(pos[1], pos[2], pos[3])
	util.BlastDamageInfo(dmginfo, pos, radius)
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} executed {2} blast [{3}][{4}] using {5}",{GAS.Logging:FormatPlayer(self.player),GAS.Logging:FormatEntity(dmg.Attacker),GAS.Logging:Highlight(tostring(dmg.Damage)),GAS.Logging:Highlight(tostring(radius)),GAS.Logging:Highlight(tostring(dmg.Inflictor))})
	end
end
...
e2function void blastDamage(entity inflictor, entity attacker, vector pos, number radius, number damage)
	...
	if GAS and GAS.Logging then
		hook.Run("KAC_Log_Wiremod","[DmgCore] {1} executed {2} blast [{3}][{4}] using {5}",{GAS.Logging:FormatPlayer(self.player),GAS.Logging:FormatEntity(attacker),GAS.Logging:Highlight(tostring(damage)),GAS.Logging:Highlight(tostring(radius)),GAS.Logging:Highlight(tostring(inflictor))})
	end
end
